...
> **This product's schema is `docs`.** Its migration ledger is
> `docs.geiger_migrations`. Substitute `docs` wherever this doc writes
...
(`docs.tickets`), never `docs.flow_tickets`.
...
-- Add seat maps
--
-- Owns docs.seat_maps. Free-form header: what this changes and why.

-- @up
create table if not exists docs.seat_maps (
...
-- @down
drop table if exists docs.seat_maps;
```
...
alter table docs.tickets add column if not exists tier text;   -- back-fill older copies
create index if not exists tickets_event_idx on docs.tickets (event_id);

drop trigger if exists tickets_touch_updated_at on docs.tickets;
create trigger tickets_touch_updated_at
before update on docs.tickets
for each row execute function docs.touch_updated_at();

drop policy if exists tickets_demo_all on docs.tickets;
create policy tickets_demo_all on docs.tickets for all to anon, authenticated
...
```sql
alter table docs.tickets enable row level security;
drop policy if exists tickets_demo_all on docs.tickets;
create policy tickets_demo_all on docs.tickets for all to anon, authenticated
  using (true) with check (true);
```
...
-- @no-transaction
-- @up
create index concurrently if not exists tickets_event_idx on docs.tickets (event_id);
```
